...
- [ ] Create `backend/src/lib/secrets.ts` with SecretsProvider interface
- [ ] Implement EnvSecretsProvider for development
- [ ] Document production secrets provider integration (AWS Secrets Manager)
- [ ] Create `backend/src/config/security.ts` with startup validation
...
- [ ] Remove bypass block from `backend/src/middleware/auth.ts` (lines 17-28)
- [ ] Remove bypass block from `backend/src/middleware/admin.ts` (lines 6-9)
- [ ] Remove bypass block from `backend/src/middleware/sse-auth.ts` (lines 26-40)
- [ ] Add compile-time guard in `backend/src/config/security.ts`
- [ ] Import security.ts in `backend/src/server.ts` before middleware
- [ ] Update test setup to use proper authentication mocking
- [ ] Verify all existing tests pass without bypass

### Day 4: CSRF Protection

- [ ] Create `backend/src/middleware/csrf.ts` (double-submit cookie pattern)
- [ ] Add `GET /api/v1/csrf-token` endpoint
- [ ] Apply `setCsrfCookie` middleware to all requests in server.ts
- [ ] Apply `verifyCsrfToken` to `/api/v1/protocols`, `/api/v1/payments`, `/api/v1/funding`
- [ ] Create `frontend/src/lib/csrf.ts` with token caching
- [ ] Update `frontend/src/lib/api.ts` to include CSRF token in state-changing requests
...
- [ ] Create Prisma migration: add `idempotencyKey` (unique) and `processedAt` to Payment model
- [ ] Run `npx prisma migrate dev --name add-payment-idempotency`
- [ ] Modify `backend/src/agents/payment/worker.ts`: replace findUnique+check with atomic `updateMany`
- [ ] Add idempotency key check in payment creation service
...
- [ ] Install pino: `npm install pino pino-pretty` (in backend/)
- [ ] Create `backend/src/lib/logger.ts` with Pino configuration
- [ ] Configure redaction paths: `*.researcherAddress`, `*.txHash`, `*.privateKey`, `*.apiKey`
- [ ] Add correlation ID middleware via AsyncLocalStorage
- [ ] Replace console.log in `backend/src/middleware/auth.ts` with logger
- [ ] Replace console.log in `backend/src/middleware/admin.ts` with logger
- [ ] Replace console.log in `backend/src/middleware/sse-auth.ts` with logger
- [ ] Replace console.log in `backend/src/middleware/errorHandler.ts` with logger
...
- [ ] Replace in-memory Map in `backend/src/middleware/rate-limit.ts` with Redis-backed store
- [ ] Verify rate limits persist across server restarts
- [ ] Reduce body size limit from 10mb to 1mb in `backend/src/server.ts`
...
- [ ] Configure Helmet with custom CSP directives in `backend/src/server.ts`
- [ ] Add HSTS with 1-year max-age, includeSubDomains, preload
