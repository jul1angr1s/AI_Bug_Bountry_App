...
redis-cli -a redis_dev_2024 KEYS "bull:*" 2>/dev/null | while read key; do
    redis-cli -a redis_dev_2024 DEL "$key" > /dev/null 2>&1
done
...
redis-cli -a redis_dev_2024 KEYS "cache:*" 2>/dev/null | while read key; do
    redis-cli -a redis_dev_2024 DEL "$key" > /dev/null 2>&1
done
redis-cli -a redis_dev_2024 KEYS "ws:*" 2>/dev/null | while read key; do
    redis-cli -a redis_dev_2024 DEL "$key" > /dev/null 2>&1
done
