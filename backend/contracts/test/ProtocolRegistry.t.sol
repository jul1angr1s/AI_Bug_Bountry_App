...
contract ProtocolRegistryTest is Test {
    ...
    function test_RegisterProtocol_RevertsOnEmptyContractPath() public {
        vm.prank(user1);
        vm.expectRevert(ProtocolRegistry.InvalidContractPath.selector);
        registry.registerProtocol(GITHUB_URL, "", CONTRACT_NAME, BOUNTY_TERMS);
    }

    function test_RegisterProtocol_RevertsOnEmptyContractName() public {
        vm.prank(user1);
        vm.expectRevert(ProtocolRegistry.InvalidContractName.selector);
        registry.registerProtocol(GITHUB_URL, CONTRACT_PATH, "", BOUNTY_TERMS);
    }

    function test_RegisterProtocol_RevertsOnDuplicateGithubUrl() public {
        // First registration
        vm.prank(user1);
        registry.registerProtocol(GITHUB_URL, CONTRACT_PATH, CONTRACT_NAME, BOUNTY_TERMS);

        // Second registration with same GitHub URL should fail
        vm.prank(user2);
        vm.expectRevert(
            abi.encodeWithSelector(
                ProtocolRegistry.ProtocolAlreadyRegistered.selector,
                GITHUB_URL
            )
        );
        registry.registerProtocol(GITHUB_URL, "src/other", "Other.sol", "Other terms");
    }
