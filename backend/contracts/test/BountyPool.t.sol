...
contract BountyPoolTest is Test {
    ...
    function test_DepositBounty_RevertsOnInvalidProtocolId() public {
        ...
        vm.expectRevert(BountyPool.InvalidProtocolId.selector);
    ...
    function test_ReleaseBounty_RevertsOnInvalidValidationId() public {
        vm.prank(payoutAgent);
        vm.expectRevert(BountyPool.InvalidValidationId.selector);
        bountyPool.releaseBounty(
            PROTOCOL_ID,
            bytes32(0),
            researcher,
    ...
    function test_ReleaseBounty_RevertsOnInvalidResearcher() public {
        vm.prank(payoutAgent);
        vm.expectRevert(BountyPool.InvalidResearcher.selector);
        bountyPool.releaseBounty(
            PROTOCOL_ID,
            VALIDATION_ID,
            address(0),
    ...
    function test_ReleaseBounty_RevertsOnDuplicateValidation() public {
        ...
        vm.prank(payoutAgent);
        vm.expectRevert(
            abi.encodeWithSelector(BountyPool.BountyAlreadyReleased.selector, VALIDATION_ID)
        );
        bountyPool.releaseBounty(
            PROTOCOL_ID,
            VALIDATION_ID,
    ...
    function test_GetBaseBountyAmount() public {
        assertEq(bountyPool.getBaseBountyAmount(), BASE_BOUNTY, "Base bounty should be 100 USDC");
    }
