...
export async function decryptProof(
  ...
  try {
    log.info(`Processing proof ${submission.proofId}`);
    ...
    log.info(`Successfully parsed proof for ${decryptedProof.vulnerabilityType}`);
    ...
    log.error(`Failed to decrypt proof:`, errorMessage);
...
export function verifyResearcherSignature(
  ...
  log.debug('Signature verification skipped in MVP');
  return true;
}
