...
const log = createLogger('ValidatorExecute');
...
export async function executeExploit(
  ...
  try {
    log.info(`Executing exploit for ${proof.vulnerabilityType}...`);
    ...
    log.info(`Exploit validation: ${validated ? 'TRUE' : 'FALSE'}`);
    ...
    log.error(`Exploit execution failed:`, errorMessage);
