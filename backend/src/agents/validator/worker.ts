...
export async function startValidatorAgent(): Promise<void> {
  ...
  log.info('Starting BullMQ worker on validation-jobs queue...');
  ...
  validatorWorker = new Worker<ProofSubmissionMessage>(
    ...
    async (job: Job<ProofSubmissionMessage>) => {
      ...
      log.debug({ proofId: submission.proofId }, 'Processing proof');
...
async function processValidation(submission: ProofSubmissionMessage): Promise<void> {
  ...
  try {
    log.debug({ proofId: submission.proofId }, 'Processing validation for proof');
    ...
    log.debug({ githubUrl: protocol.githubUrl }, 'Protocol fetched');
    ...
    log.debug({ repoPath }, 'Repository cloned');
    ...
    log.debug('Contract compiled successfully');
    ...
    log.debug({ rpcUrl: sandboxResult.rpcUrl }, 'Sandbox spawned');
    ...
    log.debug({ contractAddress: deployResult.contractAddress }, 'Contract deployed');
    ...
    log.debug({ outcome: executionResult.validated ? 'CONFIRMED' : 'REJECTED' }, 'Exploit execution completed');
    ...
    log.info({ validationStatus }, 'Validation complete');
    ...
    try {
      ...
      log.warn({ err: errMsg }, 'Agent on-chain registration failed (non-fatal)');
      log.error({ err: agentRegError }, 'Full agent registration error');
    ...
    try {
      ...
      if (!protocol || !protocol.onChainProtocolId) {
        ...
        if (!finding) {
          log.warn('Finding not found, skipping on-chain validation');
        } else {
          ...
          log.debug({ protocolId: protocol.onChainProtocolId, findingId: finding.id, severity: OnChainSeverity[onChainSeverity], outcome: ValidationOutcome[outcome] }, 'Recording validation on-chain...');
          ...
          log.debug('Database updated with on-chain validation IDs');
      ...
      log.error({ err: errMsg }, 'Failed to record validation on-chain');
      log.error({ err: onChainError }, 'Full on-chain error');
    ...
    try {
      ...
      if (!finding) {
        log.warn('Finding not found, skipping reputation feedback');
      } else {
        ...
        if (researcherWallet && validatorWallet) {
          ...
          log.debug({ researcherWallet: researcherWallet || '(not found)', validatorWallet: validatorWallet || '(not found)' }, 'Skipping reputation feedback - agent wallets not found');
        }
      }
    } catch (reputationError) {
      const errMsg = reputationError instanceof Error ? reputationError.message : String(reputationError);
      log.error({ err: errMsg }, 'Failed to record reputation feedback');
      log.error({ err: reputationError }, 'Full reputation error');
    ...
    log.error({ err: errorMessage }, 'Validation failed');
    ...
    if (anvilProcess) {
      try {
        ...
        log.error({ err: error }, 'Failed to kill sandbox');
      }
    }
