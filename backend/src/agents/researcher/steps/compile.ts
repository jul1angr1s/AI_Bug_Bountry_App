...
const log = createLogger('ResearcherCompile');
...
export async function executeCompileStep(params: CompileStepParams): Promise<CompileStepResult> {
  ...
  try {
    log.info(`Compiling contracts in ${clonedPath}...`);
    ...
    try {
      ...
      log.info('No foundry.toml found, will use default settings');
    }

    // Initialize Foundry project if needed
    if (!hasFoundryConfig) {
      log.info('Initializing Foundry project...');
      try {
        await execAsync('forge init --force --no-git', { cwd: clonedPath });
      } catch (initError) {
        log.warn('Init failed, continuing anyway:', initError);
      }
    }
    ...
    try {
      ...
      log.warn('Dependency installation failed, continuing:', installError);
    }
    ...
    log.info('Build completed, extracting artifacts...');
    ...
    log.info(`Successfully compiled ${contractName}`);
...
async function extractArtifacts(
  ...
  try {
    ...
    log.debug(`Looking for artifact at ${artifactPath}`);
    ...
    if (!abi) {
      log.warn('No ABI found in artifact');
    }

    if (!bytecode) {
      log.warn('No bytecode found in artifact');
    }

    return { abi, bytecode };

  } catch (error) {
    log.error('Failed to extract artifacts:', error);

    // Try to find the artifact in alternative locations
    try {
      const files = await findJsonFiles(artifactsPath);
      log.debug(`Found ${files.length} artifact files, searching for ${contractName}...`);
      ...
      for (const file of files) {
        if (file.includes(contractName)) {
          ...
          if (artifact.abi && artifact.bytecode) {
            log.debug(`Found artifact at ${file}`);
            return {
              abi: artifact.abi,
      ...
      log.error('Failed to search for artifacts:', searchError);
    }
