...
export async function executeCloneStep(params: CloneStepParams): Promise<CloneStepResult> {
  ...
  try {
    ...
    console.log(`[Clone] Cloning ${repoUrl} to ${clonePath}...`);
    ...
    if (targetCommitHash) {
      console.log(`[Clone] Checking out commit ${targetCommitHash}...`);
    ...
    console.log(`[Clone] Successfully cloned to ${clonePath} at commit ${actualCommitHash}`);
    ...
    try {
      ...
      console.error('[Clone] Failed to clean up after error:', cleanupErr);
    }
