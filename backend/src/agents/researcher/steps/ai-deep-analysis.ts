...
export async function executeAIDeepAnalysisStep(
  ...
  log.info({ contractName }, 'Starting AI analysis');
  ...
  try {
    log.info('Starting AI-powered analysis');
    ...
    try {
      ...
      log.debug({ chars: contractSource.length }, 'Read contract source');
    } catch (readError) {
      log.error({ err: readError }, 'Failed to read contract source');
    ...
    if (slitherFindings.length > 0) {
      log.info({ count: slitherFindings.length }, 'Enhancing Slither findings');
      ...
      for (const finding of slitherFindings) {
        try {
          ...
          log.warn({ err: enhanceError }, 'Failed to enhance finding, using original');
    ...
    log.info('Searching for new vulnerabilities with AI');
