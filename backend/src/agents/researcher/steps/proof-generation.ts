...
const log = createLogger('ProofGeneration');
...
export async function executeProofGenerationStep(
  ...
  log.info(`Generating proofs for ${findings.length} findings...`);
  ...
  for (const finding of findings) {
    try {
      ...
      log.debug(`Generated proof for finding ${finding.id}`);
    } catch (error) {
      log.error(`Failed to generate proof for finding ${finding.id}:`, error);
      // Continue with other findings
    }
  }

  log.info(`Generated ${proofs.length} proofs`);
