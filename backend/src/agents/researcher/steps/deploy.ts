...
const log = createLogger('ResearcherDeploy');
...
export async function executeDeployStep(params: DeployStepParams): Promise<DeployStepResult> {
  ...
  log.info(`Starting Anvil on port ${freePort}...`);
  ...
  try {
    ...
    log.info(`Anvil ready on port ${freePort}`);
    ...
    log.info(`Contract deployed at ${deploymentAddress}`);
    ...
    log.error('Deployment failed, cleaning up Anvil...');
...
function startAnvil(port: number): Promise<ChildProcess> {
  return new Promise((resolve, reject) => {
    log.debug(`Spawning anvil on port ${port}...`);
    ...
    anvil.stdout?.on('data', (data) => {
      ...
      outputBuffer += output;
      log.debug('Anvil stdout:', output.trim());
      ...
      if ((output.includes('Listening') || output.includes('localhost:') || outputBuffer.includes('Accounts')) && !started) {
        started = true;
        log.info('Anvil started successfully!');
    ...
    setTimeout(() => {
      if (!started) {
        log.error('Anvil startup timeout after 15 seconds');
        anvil.kill();
  ...
  return new Promise((resolve) => {
    ...
    const timeout = setTimeout(() => {
      if (!anvilProcess.killed) {
        log.warn('Anvil did not stop gracefully, forcing kill...');
    ...
    anvilProcess.on('exit', () => {
      ...
      log.info('Anvil process terminated');
      resolve();
    });
  });
}
