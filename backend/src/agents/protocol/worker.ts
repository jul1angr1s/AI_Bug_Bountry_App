...
const log = createLogger('ProtocolAgent');
...
export async function processProtocolRegistration(
  ...
  log.info({ jobId: job.id, protocolId }, 'Processing registration job');
  ...
  try {
    ...
    log.debug({ contractFullPath: verifyResult.contractFullPath }, 'Contract verified');
    ...
    log.debug({ bytecodeSize: compileResult.bytecode!.length / 2 }, 'Contract compiled successfully');
    ...
    log.debug({ riskScore }, 'Risk score calculated');
    ...
    if (skipOnChainRegistration) {
      log.info('Skipping on-chain registration (SKIP_ONCHAIN_REGISTRATION=true)');
      ...
      log.debug({ derivedOnChainProtocolId }, 'Derived onChainProtocolId');
      ...
      // Each version gets its own on-chain protocolId
      ...
      log.info({ onChainProtocolId: onChainResult.protocolId, txHash: onChainResult.txHash, block: onChainResult.blockNumber }, 'On-chain registration successful');
    ...
    try {
      ...
      log.info({ protocolId }, 'Protocol set to AWAITING_FUNDING — owner must fund the bounty pool before requesting scans');
      ...
      log.error({ err: fundingError }, 'Failed to set AWAITING_FUNDING state');
    ...
    log.info({ protocolId }, 'Registration completed successfully');
    ...
    log.error({ protocolId, err: errorMessage }, 'Registration failed');
