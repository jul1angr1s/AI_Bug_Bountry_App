...
export async function registerProtocol(
  ...
  try {
    // Find existing versions of this GitHub URL to determine next version
    const existingVersions = await prisma.protocol.findMany({
    ...
    const registrationType = nextVersion === 1 ? 'INITIAL' : 'DELTA';
    const parentProtocolId = existingVersions.length > 0
      ? existingVersions.find(v => v.version === 1)?.id ?? null
      : null;
    ...
    return {
      ...
      protocol: {
        ...
        registrationType: protocol.registrationType,
        parentProtocolId: protocol.parentProtocolId,
      },
    };
  } catch (error) {
    log.error({ err: error }, 'Error registering protocol');
    return {
      success: false,
      error: {
...
export async function fundProtocol(
  ...
  try {
    ...
    log.error({ err: error }, 'Error funding protocol');
    return {
      success: false,
      error: {
        code: 'FUNDING_FAILED',
...
export async function updateProtocolRegistrationState(
  ...
  try {
    ...
    log.error({ err: error }, 'Error updating protocol registration state');
  }
}
...
export async function confirmFundingEvent(
  ...
  try {
    ...
    log.error({ err: error }, 'Error confirming funding event');
  }
}
