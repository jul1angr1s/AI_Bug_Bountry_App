...
export async function verifyProtocolFunding(
  ...
  try {
    ...
    log.error({ err: error }, 'Failed to fetch on-chain balance');
    return {
      success: false,
  ...
  try {
    ...
    log.error({ err: error }, 'Failed to emit WebSocket event');
  }
...
export async function requestScan(
  ...
  try {
    ...
    log.info({ scanId: scan.id, protocolId }, 'Created scan for protocol');
    ...
    log.error({ err: error }, 'Failed to create scan');
    return {
      success: false,
      error: {
...
export async function setAwaitingFunding(protocolId: string): Promise<void> {
  ...
  log.info({ protocolId }, 'Protocol set to AWAITING_FUNDING');
}
