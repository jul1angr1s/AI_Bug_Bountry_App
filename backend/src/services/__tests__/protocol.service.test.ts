...
describe('ProtocolService', () => {
  ...
  describe('registerProtocol', () => {
    it('registers a new protocol successfully with version 1', async () => {
      ...
      expect(result.protocol!.registrationType).toBe('INITIAL');
    });

    it('first registration gets version 1 and INITIAL type', async () => {
      const input = makeRegistrationInput();
      ...
      expect(createCall.data.registrationType).toBe('INITIAL');
      expect(createCall.data.parentProtocolId).toBeNull();
    });

    it('registers same githubUrl with incremented version', async () => {
      const input = makeRegistrationInput();
      const existingProtocol = makeProtocol({ id: 'proto-001', version: 1 });

      mockPrisma.protocol.findMany.mockResolvedValue([existingProtocol]);
      mockPrisma.$transaction.mockImplementation(async (cb: any) => cb(mockPrisma));
      mockPrisma.protocol.create.mockResolvedValue({
        id: 'proto-new',
        githubUrl: input.githubUrl,
      ...
      expect(result.success).toBe(true);
      expect(result.protocol!.version).toBe(2);
      expect(result.protocol!.registrationType).toBe('DELTA');

      const createCall = mockPrisma.protocol.create.mock.calls[0][0];
      expect(createCall.data.version).toBe(2);
      expect(createCall.data.registrationType).toBe('DELTA');
    });

    it('auto-increments version based on existing count', async () => {
      const input = makeRegistrationInput();
      const existingProtocols = [
        makeProtocol({ id: 'proto-003', version: 3 }),
        makeProtocol({ id: 'proto-002', version: 2 }),
        makeProtocol({ id: 'proto-001', version: 1 }),
      ];

      mockPrisma.protocol.findMany.mockResolvedValue(existingProtocols);
      mockPrisma.$transaction.mockImplementation(async (cb: any) => cb(mockPrisma));
      mockPrisma.protocol.create.mockResolvedValue({
        id: 'proto-new',
        githubUrl: input.githubUrl,
    ...
    it('links via parentProtocolId', async () => {
      const input = makeRegistrationInput();
      const existingProtocol = makeProtocol({ id: 'proto-001', version: 1 });
      ...
      const createCall = mockPrisma.protocol.create.mock.calls[0][0];
      expect(createCall.data.parentProtocolId).toBe('proto-001');
    });
    ...
    it('creates an audit log entry on successful registration', async () => {
      ...
      expect(auditCall.data.metadata.version).toBe(1);
    });
    ...
    it('does not create protocol when findMany throws', async () => {
      const input = makeRegistrationInput();
      mockPrisma.protocol.findMany.mockRejectedValue(new Error('DB error'));
