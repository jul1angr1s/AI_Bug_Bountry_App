...
const log = createLogger('AgentReputationRegistry');
...
export class AgentReputationRegistryClient {
  ...
  async initializeReputation(
    ...
    log.info('Initializing reputation on-chain...');
    ...
    log.info(`Reputation initialized: tx=${tx.hash}`);
  ...
  async recordFeedback(
    ...
    log.info('Recording feedback on-chain...');
    log.debug(`Researcher Agent ID: ${researcherAgentId}`);
    log.debug(`Validator Agent ID: ${validatorAgentId}`);
    log.debug(`Feedback: ${FeedbackType[feedbackType]}`);
    ...
    log.info(`Feedback recorded: id=${feedbackId}, tx=${tx.hash}`);
  ...
  async recordValidatorFeedback(
    ...
    log.info('Recording validator feedback on-chain...');
    log.debug(`Validator Agent ID: ${validatorAgentId}`);
    log.debug(`Researcher Agent ID: ${researcherAgentId}`);
    log.debug(`Feedback: ${FeedbackType[feedbackType]}`);
    ...
    log.info(`Validator feedback recorded: id=${feedbackId}, tx=${tx.hash}`);
