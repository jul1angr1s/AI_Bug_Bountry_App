...
const log = createLogger('AgentIdentityRegistry');
...
export class AgentIdentityRegistryClient {
  ...
  async registerAgent(
    ...
    log.info('Registering agent on-chain...');
    log.info(`  Address: ${walletAddress}`);
    log.info(`  Type: ${AgentType[agentType]}`);
    ...
    log.info(`Agent registered: tokenId=${tokenId}, tx=${tx.hash}`);
