...
router.post('/siwe', async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    log.info({ wallet: normalizedAddress }, 'SIWE signature verified');
    ...
    try {
      ...
      if (existingUser) {
        ...
        log.info({ userId }, 'Updated existing user');
      } else {
        ...
        if (createError) {
          ...
          if ((createError as any).code === 'email_exists') {
            ...
            if (foundUser) {
              ...
              userId = foundUser.id;
              log.info({ userId }, 'Recovered existing user');
            } else {
              throw createError;
            }
          } else {
      ...
      log.error({ err: error }, 'Supabase user operation failed');
      throw error;
    }
    ...
    try {
      ...
      log.info({ userId }, 'Generated session tokens');
    } catch (error) {
      log.error({ err: error }, 'Token generation failed');
      throw error;
    }
