...
const log = createLogger('ScanRoutes');
const router = Router();
  ...
  try {
    ...
    if (researcherWallet) {
      try {
        ...
        log.info(`Submission fee deducted for researcher ${researcherWallet}, scan ${scan.id}`);
      } catch (feeError) {
        log.error('Fee deduction failed (scan still created):', feeError);
      }
    }
