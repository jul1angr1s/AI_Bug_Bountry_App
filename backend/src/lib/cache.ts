...
export async function getCache<T>(key: string): Promise<T | null> {
  try {
    ...
    log.error({ err: error }, 'Cache get error');
    return null;
  }
}
...
export async function setCache<T>(key: string, value: T, ttlSeconds: number): Promise<void> {
  try {
    ...
    log.error({ err: error }, 'Cache set error');
  }
}
...
export async function invalidateCache(key: string): Promise<void> {
  try {
    ...
    log.error({ err: error }, 'Cache invalidate error');
  }
}
...
export async function invalidateCachePattern(pattern: string): Promise<void> {
  try {
    ...
    log.error({ err: error }, 'Cache pattern invalidate error');
  }
}
...
export async function getCacheStats(): Promise<{ hits: number; misses: number; keys: number }> {
  try {
    ...
    log.error({ err: error }, 'Cache stats error');
