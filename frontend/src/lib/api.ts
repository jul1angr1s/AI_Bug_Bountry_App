...
/**
 * Fetch protocol versions for a GitHub URL
 */
export async function fetchProtocolVersions(githubUrl: string): Promise<Array<{
  id: string;
  version: number;
  registrationType: string;
  status: string;
  ...
  const headers = await getAuthHeaders();
  const response = await fetch(
    `${API_BASE_URL}/api/v1/protocols/versions?githubUrl=${encodeURIComponent(githubUrl)}`,
    { headers, credentials: 'include' }
  );
