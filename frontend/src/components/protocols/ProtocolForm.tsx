...
import { fetchProtocolVersions } from '../../lib/api';
...
export default function ProtocolForm({ onSubmit, isSubmitting = false, initialValues }: ProtocolFormProps) {
  ...
  // Debounced version check on GitHub URL change
  const checkExistingVersions = useCallback(async (url: string) => {
    const githubPattern = /^https:\/\/github\.com\/[\w-]+\/[\w.-]+$/;
    if (!githubPattern.test(url)) {
      setExistingVersionCount(0);
      return;
    }

    setVersionCheckLoading(true);
    try {
      const versions = await fetchProtocolVersions(url);
      setExistingVersionCount(versions.length);
    } catch {
      setExistingVersionCount(0);
    } finally {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      if (formData.githubUrl) {
        checkExistingVersions(formData.githubUrl);
      } else {
        setExistingVersionCount(0);
      }
    }, 500);
    return () => clearTimeout(timer);
  }, [formData.githubUrl, checkExistingVersions]);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div>
        ...
        {!errors.githubUrl && existingVersionCount > 0 && (
          <div className="flex items-center gap-2 mt-2 px-3 py-2 bg-cyan-500/10 border border-cyan-500/20 rounded-lg">
            <MaterialIcon name="info" className="text-sm text-cyan-400" />
            <p className="text-sm text-cyan-400">
              This URL has {existingVersionCount} existing version{existingVersionCount !== 1 ? 's' : ''}. Registering will create version {existingVersionCount + 1} (delta audit).
            </p>
          </div>
        )}
        {versionCheckLoading && (
          <div className="flex items-center gap-2 mt-2">
            <MaterialIcon name="progress_activity" className="text-sm text-gray-400 animate-spin" />
            <p className="text-sm text-gray-400">Checking for existing versions...</p>
          </div>
        )}
      </div>
