  ...
  if (isLoading) {
    return (
      <div className="bg-navy-800 rounded-xl border border-navy-700 p-6">
        <div className="h-6 bg-navy-700 rounded w-1/3 mb-6 animate-pulse"></div>
        <div className="grid grid-cols-4 h-[200px] items-end gap-4 px-2">
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="w-full bg-navy-700 rounded-t-lg animate-pulse" style={{ height: `${30 + i * 20}%` }}></div>
          ))}
        </div>
      </div>
    );
  ...
  return (
    <div className="bg-navy-800 rounded-xl border border-navy-700 p-6">
      <div className="flex justify-between items-start mb-6">
        <div>
          <h3 className="text-lg font-bold text-white">Payout Distribution by Severity</h3>
          <p className="text-gray-400 text-sm">Current bounty amounts per severity level</p>
        </div>
      </div>
        ...
        {severityData.map((data, index) => {
          ...
          return (
            ...
            >
              ...
              <p className="text-xs font-medium text-gray-400 mt-2">{data.label}</p>
            </div>
          );
        })}
      </div>
